<script lang="ts">
    ...
    $: percentageText =
        Math.round((good.length / _starred.length) * 10000) / 100;
    ...
    questions.subscribe(async (qs) => {
        if (!browser) return;
        wrong = JSON.parse(localStorage.getItem("sWrong") ?? "[]");
        good = JSON.parse(localStorage.getItem("sGood") ?? "[]");

        wrong = [...new Set(wrong)];
        good = [...new Set(good)];
        localStorage.setItem("sWrong", JSON.stringify(wrong));
        localStorage.setItem("sGood", JSON.stringify(good));
    ...
    function updateProgressBar() {
        let percentage = good.length / $starred.length;
...
<QuestionsHandler>
    <div class="w-full h-6 rounded-lg border border-white relative">
        ...
        >
            {good.length}/{_starred.length} ({percentageText}%)
        </div>
